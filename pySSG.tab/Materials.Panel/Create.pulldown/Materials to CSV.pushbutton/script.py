...
if selected_folder:
    ...
    mat_schemas = {
        'PrismLayeredSchema': ['LayeredDiffuse', 'LayeredBottomF0', 'SurfaceNormal', 'LayeredNormal'],
    ...
    selected_mat = forms.SelectFromList.show(
        {
            ...
            'Generic': [MatOption(x) for x in col if 'GenericSchema' in revit.doc.GetElement(x.AppearanceAssetId).GetRenderingAsset().Name],            
    ...
    if selected_mat:
        ...
        for mat in selected_mat:
            ...
            # Description
            mat_dict['Description'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_DESCRIPTION).AsString()

            # Material Class
            mat_dict['Class'] = mat.MaterialClass

            # Comments
            mat_dict['Comments'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS).AsString()

            # Keywords cannot be accessed through API

            # Manufacturer
            mat_dict['Manufacturer'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_MANUFACTURER).AsString()

            # Model
            mat_dict['Model'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_MODEL).AsString()

            # Cost
            mat_dict['Cost'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_COST).AsDouble()

            # URL
            mat_dict['URL'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_URL).AsString()

            # Keynote
            mat_dict['Keynote'] = mat.get_Parameter(DB.BuiltInParameter.KEYNOTE_PARAM).AsString()

            # Mark
            mat_dict['Mark'] = mat.get_Parameter(DB.BuiltInParameter.ALL_MODEL_MARK).AsString()
            ...
            with DB.Visual.AppearanceAssetEditScope(assetElem.Document) as editScope:
                ...
                # Asset Name
                mat_dict['Asset Name'] = editableAsset[DB.Visual.SchemaCommon.UIName].Value
                
                # Asset Description
                mat_dict['Asset Description'] = editableAsset[DB.Visual.SchemaCommon.Description].Value
    
                # Keywords
                mat_dict['Asset Keywords'] = editableAsset[DB.Visual.SchemaCommon.Keyword].Value

                # Schema
                mat_dict['Base Schema'] = editableAsset[DB.Visual.SchemaCommon.BaseSchema].Value
                ...
                try:
                    ...
                    for schema_prop in appProps:
                        ...
                        prop = editableAsset[prop_by_name(mat_dict['Base Schema'], schema_prop)]
                        ...
                        if prop.Type == DB.Visual.AssetPropertyType.Double4:
                            ...
                            if not connectedAsset:
                                ...
                                mat_dict[schema_prop] = ",".join([str(colorByte.Red), str(colorByte.Blue), str(colorByte.Green)])
                            else:
        ...
        try:
            with open(selected_folder+'\\revit_mats.csv', 'w') as new_file:
                ...
                forms.toast(
                    ...
                    actions={
                        "Open File": selected_folder+'\\revit_mats.csv'
                        },
                )
        except IOError:
