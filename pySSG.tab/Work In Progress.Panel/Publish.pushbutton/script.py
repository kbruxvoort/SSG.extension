...
def get_value(fam_type, param):
    ...
    if param.StorageType == DB.StorageType.Double:
        ...
        _id = fam_type.AsElementId(param)
        e = revit.doc.GetElement(_id)
        if e:
            value = e.Name
    return value

client = WebClient()
client.Headers.Add("Authorization", settings.DEV_HEADERS['Authorization'])
client.Headers.Add("Accept", "application/json")
client.Headers.Add("Content-Type", "application/json")
url = settings.POST_FAMILY_URL
...
if ssgfid:
    _id = get_value(default_type, ssgfid[0])
    if _id:
        exists = True
        print('\tIdentity parameters exist. Getting parameter list from database...')
        get_url = settings.GET_FAMILY + _id
        business_fams = json.loads(client.DownloadString(get_url)).get('BusinessFamilies')
        if business_fams:
        ...
        exists = False
        type_params = forms.select_family_parameters(revit.doc, 
                                                        title='Select Family Type Parameters',
                                                        include_instance=False)
        if type_params:
            type_params_names = [x.Definition.Name for x in type_params]
            global_params = forms.select_family_parameters(revit.doc, 
    ...
    if id_group:
        ...
        with revit.Transaction("Add Identity Parameters"):
            print('\tAdding SSGFID')
            fam_id_param = fam_mgr.AddParameter(ssgfid_param[0], DB.BuiltInParameterGroup.PG_IDENTITY_DATA, False)
            print('\tAdding SSGTID')
            type_id_param = fam_mgr.AddParameter(ssgtid_param[0], DB.BuiltInParameterGroup.PG_IDENTITY_DATA, False)
            ...
            if not zO_param:
                ...
                if other_group:
                    ...
                    print('\tAdding zO')
                    zO_param = fam_mgr.AddParameter(zO_param[0], DB.BuiltInParameterGroup.INVALID, False)
            else:
                zO_param = zO_param[0]
        ...
        raise Exception('This shared parameter file does not have the correct group [Identity Data]')
    ...
    for p in params:
        if not p.IsInstance:
            if p.Definition.Name in global_params_names:
                ...
                gp = Parameter(p.Definition.Name, value, DataType=str(p.Definition.ParameterType), ParameterType='Type')
    ...
    for fam_type, db_type, zO in type_matches:
        ...
        for p in params:
            if p.Definition.Name in type_params_names:
                ...
                tp = Parameter(p.Definition.Name, value, DataType=str(p.Definition.ParameterType), ParameterType='Type')
                db_type.Parameters.append(tp)

    for i in instance:
        if i.Definition.Name in global_params_names:
            value = get_value(default_type, i)
            ip = Parameter(i.Definition.Name, value, DataType=str(i.Definition.ParameterType), ParameterType='Instance')
...
print('Family Published to Database')