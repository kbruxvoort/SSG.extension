"""
This script changes the look of reference planes to make it easy to visually identify the Is Reference parameter.
"""
import clr
import Autodesk
...
with revit.Transaction("Create Subcategory"):
    ...
    try:
        if "Named References" not in subNames:
            named_subcat = revit.doc.Settings.Categories.NewSubcategory(ref_plane_cat, "Named References")
            named_subcat.LineColor = DB.Color(128, 0, 255)
            named_subcat.SetLinePatternId(ref_plane_cat.GetLinePatternId(DB.GraphicsStyleType.Projection), DB.GraphicsStyleType.Projection)
            named_subcat_id = named_subcat.Id
        if "Weak References" not in subNames:
            weak_subcat = revit.doc.Settings.Categories.NewSubcategory(ref_plane_cat, "Weak References")
            weak_subcat.LineColor = DB.Color(255, 0, 128)
            weak_subcat.SetLinePatternId(ref_plane_cat.GetLinePatternId(DB.GraphicsStyleType.Projection), DB.GraphicsStyleType.Projection)
            weak_subcat_id = weak_subcat.Id
        if "Not References" not in subNames:
            not_subcat = revit.doc.Settings.Categories.NewSubcategory(ref_plane_cat, "Not References")
            not_subcat.LineColor = DB.Color(0, 128, 255)
            not_subcat.SetLinePatternId(ref_plane_cat.GetLinePatternId(DB.GraphicsStyleType.Projection), DB.GraphicsStyleType.Projection)
            not_subcat_id = not_subcat.Id

        for c in col:
        # print(c.get_Parameter(DB.BuiltInParameter.ELEM_REFERENCE_NAME)).AsString()
        # print(c.get_Parameter(DB.BuiltInParameter.ELEM_REFERENCE_NAME_2D_XZ)).AsValueString()
        # print(c.get_Parameter(DB.BuiltInParameter.ELEM_REFERENCE_NAME_2D_XZ).AsValueString())
        
            if c.get_Parameter(DB.BuiltInParameter.ELEM_REFERENCE_NAME_2D_XZ).AsValueString() == "Not a Reference":
                not_ref.append(c)

            elif c.get_Parameter(DB.BuiltInParameter.ELEM_REFERENCE_NAME_2D_XZ).AsValueString() == "Weak Reference":
                weak_ref.append(c)
