"""
This script populates a spec guide PDF with parameters from a selected family. Your active view must be a 3D View.
"""
import itertools
import math
...
sched_loc = DB.XYZ(.81640625/12, 10.08984375/12, 0)
...
if source_elements:
    ...
    mfg_value_rule = DB.FilterStringRule(mfg_param_prov, param_contains,"Southwest Solutions Group", False)
    param_filter = DB.ElementParameterFilter(mfg_value_rule)
    ssg_fams = DB.FilteredElementCollector(revit.doc).WherePasses(param_filter).WhereElementIsNotElementType().ToElements()
    ...
    pdf_tb = [t for t in title_blocks if t.FamilyName + ": " + t.LookupParameter("Type Name").AsString() == "SSG_TB_8.5x11_PDF: Primary"][0]
    pdf_tb_2 = [t for t in title_blocks if t.FamilyName + ": " + t.LookupParameter("Type Name").AsString() == "SSG_TB_8.5x11_PDF: Empty"][0]
        ...
        for idx, source_element in enumerate(source_elements):
            with revit.Transaction("Create Spec PDF"):
                ...
                if ele_name in overflow:
                    ...
                    sheet_scheds = DB.FilteredElementCollector(revit.doc, sheet_2.Id).OfClass(DB.ScheduleSheetInstance).ToElements()
                    ...
                    if not sheet_scheds:
                        if name_2 in schedule_names:
                            ...
                            print("\tSchedule 2 already exists. Placing schedule 2 on sheet 2")
                        else:
                            # Duplicate base schedule and set filter
                            new_sched_id = spec_schedules[0].Duplicate(DB.ViewDuplicateOption.Duplicate)   
                            sched_2 = revit.doc.GetElement(new_sched_id)    
                            sched_2.ViewName = name_2
                            tgt_filter = DB.ScheduleFilter(fields[0], ref_filt.FilterType, name_2)
                        ...
                        sched_inst_2 = DB.ScheduleSheetInstance.Create(revit.doc, sheet_2.Id, sched_2.Id, sched_loc)
                    else:
                    ...
                    issue_date = sheet_2.get_Parameter(DB.BuiltInParameter.SHEET_ISSUE_DATE)
                ...
                # Check for schedules on sheet
                sheet_scheds = DB.FilteredElementCollector(revit.doc, sheet_1.Id).OfClass(DB.ScheduleSheetInstance).ToElements()
                # If there is no schedule on the sheet see if it already exists in the project
                if not sheet_scheds:
                    if name_1 in schedule_names:
                        sched_1 = [s for s in all_schedules if s.Name == name_1][0]
                        print("\tSchedule 1 already exists. Placing schedule 1 on sheet 1")
                    else:
                        # Duplicate base schedule and set filter
                        new_sched_id = spec_schedules[0].Duplicate(DB.ViewDuplicateOption.Duplicate)   
                        sched_1 = revit.doc.GetElement(new_sched_id)    
                        sched_1.ViewName = name_1
                        tgt_filter = DB.ScheduleFilter(fields[0], ref_filt.FilterType, name_1)
                        new_sched_def = sched_1.Definition
                        new_sched_def.SetFilter(0,tgt_filter)
                        print("\tCreating Schedule 1 and placing on sheet 1")
                    # Place schedule on sheet
                    sched_inst_1 = DB.ScheduleSheetInstance.Create(revit.doc, sheet_1.Id, sched_1.Id, mid_right)
                else:
                ...
                '''
                # Check if sheet already exists for this family
                if "SPEC_" + ele_name in sheet_names:
                    sheet_1 = [s for s in all_sheets if s.Name == "SPEC_" + ele_name + "-1"][0]

                    # Get all viewports on sheet
                    viewports = sheet.GetAllViewports()

                    # If the 3d exists but isn't on the sheet add it to current sheet and apply view template
                    if not viewports:
                        if ele_name in names_3d:
                            view_3d = [v for v in all_3d if v.ViewName == ele_name][0]
                            print("\t3D view already exists. Placing view on sheet")
                        else:
                            ...
                            view_3d.Name = ele_name
                            print('\tCreating 3D view and placing on sheet')
                            revit.doc.Regenerate()
                        ...
                        print("\t3D is already placed on sheet")


                    sheet_scheds = DB.FilteredElementCollector(revit.doc, sheet.Id).OfClass(DB.ScheduleSheetInstance).ToElements()
                    if not sheet_scheds:
                        if "SPEC_" + ele_name + "-1" in schedule_names:
                            sched_1 = [s for s in all_schedules if s.Name == "SPEC_" + ele_name + "-1"][0]
                            print("\tSchedule already exists. Placing schedule on sheet")
                            ...
                            if ele_name in overflow:
                                new_sched_id_1 = spec_schedules[0].Duplicate(DB.ViewDuplicateOption.Duplicate)   
                                new_sched_id_2 = spec_schedules[0].Duplicate(DB.ViewDuplicateOption.Duplicate)   
                                sched_1 = revit.doc.GetElement(new_sched_id)    
                                sched_1.ViewName = 'SPEC_' + ele_name + "-1"
                                tgt_filter_1 = DB.ScheduleFilter(fields[0], ref_filt.FilterType, ele_name)
                                tgt_filter_2 = DB.ScheduleFilter(fields[0], ref_filt.FilterType, ele_name)
                                new_sched_def_1 = sched.Definition
                                new_sched_def_2 = sched.Definition
                                new_sched_def_1.SetFilter(0,tgt_filter_1)
                                new_sched_def_2.SetFilter(0,tgt_filter_2)

                                sheet_1 = DB.ViewSheet.Create(revit.doc, pdf_tb.Id)
                                sheet_1.SheetNumber = str(max_num+idx+1)
                                sheet_1.Name = "SPEC_" + ele_name + "-1"

                                sheet_2 = DB.ViewSheet.Create(revit.doc, pdf_tb.Id)
                                sheet_2.SheetNumber = str(max_num+idx+1)
                                sheet_2.Name = "SPEC_" + ele_name + "-2"

                                print('\tCreating schedule and placing on sheet')

                            else:
                                new_sched_id = spec_schedules[0].Duplicate(DB.ViewDuplicateOption.Duplicate)   
                                sched = revit.doc.GetElement(new_sched_id)    
                                sched.ViewName = 'SPEC_' + ele_name
                                tgt_filter = DB.ScheduleFilter(fields[0], ref_filt.FilterType, ele_name)
                                new_sched_def = sched.Definition
                                new_sched_def.SetFilter(0,tgt_filter)
                                print('\tCreating schedule and placing on sheet')

                        if sheet_2:
                            sched_inst_1 = DB.ScheduleSheetInstance.Create(revit.doc, sheet_1.Id, sched_1.Id, mid_right)
                            sched_inst_2 = DB.ScheduleSheetInstance.Create(revit.doc, sheet_2.Id, sched_2.Id, top_left)
                        else:
                            sched_inst = DB.ScheduleSheetInstance.Create(revit.doc, sheet.Id, sched.Id, mid_right)

                    else:
                        print("\tSchedule is already placed on sheet")

                else:
                    # with revit.Transaction("Create Sheets per Element"):
                    sheet = DB.ViewSheet.Create(revit.doc, pdf_tb.Id)
                    sheet.SheetNumber = str(max_num+idx+1)
                    sheet.Name = "SPEC_" + ele_name
                    ...
                    view_3d.ViewTemplateId = primary_template[0].Id
                    revit.doc.Regenerate()
                    vp = DB.Viewport.Create(revit.doc, sheet.Id, view_3d.Id, top_left)
                    vp.ChangeTypeId(newVp.Id)

                    if "SPEC_" + ele_name in schedule_names:
                            sched = [s for s in all_schedules if s.Name == "SPEC_" + ele_name + "-1"][0]
                            print("\t Schedule already exists. Placing schedule on new sheet")

                    else:
                        
                        # Duplicate SPEC schedule and apply family name filter
                        ref_def = spec_schedules[0].Definition
                        ref_filt = ref_def.GetFilter(0)
                        field_ids = ref_def.GetFieldOrder()
                        fields = []
                        ...
                        # Check number of rows in case you need to split
                        ...
                        print('\tCreating schedule and placing on new sheet')

                    sched_inst = DB.ScheduleSheetInstance.Create(revit.doc, sheet.Id, sched.Id, mid_right)
                    
                    
                    for k, v in param_dict.items():
                        p = sheet.LookupParameter(v)
                        value = source_element.LookupParameter(k)
                        if not value:
                            value = ele_type.LookupParameter(k)
