"""
This script creates a Legend by selecting the family instances you want to populate the PDF. Your active view must be a plan view.
"""
import math
import itertools
import rpw
...
multi_tags = DB.FilteredElementCollector(revit.doc).OfCategory(
            DB.BuiltInCategory.OST_MultiCategoryTags).WhereElementIsElementType().\
            ToElements()
...
components = [Label('Border Size'), 
            ComboBox('combobox1', {'1/8"': .125/12,'1/4"': .25/12, '1/2"': .5/12, '3/4"':.75/12}, default='1/2"'),
            Label('Tag Type'),
            ComboBox('combobox2', list_to_dict_2(multi_tags), default="SSG_Tag_Multicategory: SSG_Prefix"),
            Label('Titleblock'),
            ComboBox('combobox3', list_to_dict_2(title_blocks), default="SSG_TB_8.5x11_PDF: Empty"),
            Label('View Template'),
            ComboBox('combobox4', list_to_dict(view_templates), default="SSG_Elevation - Legend"),
            Label('Separator Line Style'),
            ComboBox('combobox6', list_to_dict(all_text), default='SSG_3/32" - Segoe UI Semibold - Dark Blue'),
            Label('Sheet Header'),
...
if form.show():
    ...
    if source_elements:
        ...
        # Get default text
        default_text_id = revit.doc.GetDefaultElementTypeId(DB.ElementTypeGroup.TextNoteType)


        # Get Interior Elevation View Type
        v_types = []
        view_types = DB.FilteredElementCollector(revit.doc).OfClass(DB.ViewFamilyType).ToElements()
        for v in view_types:
        ...
        vps = []
        ele_types = DB.FilteredElementCollector(revit.doc).OfClass(DB.ElementType).ToElements()
        for c in ele_types:
        ...
        with revit.Transaction("Create Legend Elevations"):
            ...
            for group in fam_groups:
                ...
                for ele in group:
                    ...
                    eleMarker = DB.ElevationMarker.CreateElevationMarker(revit.doc, viewType.Id, marker_loc, 100)
                    ...
                    viewBB.Min = DB.XYZ(marker_loc.X - width/2 - crop_offset, marker_loc.Z - crop_offset, 0)
                    viewBB.Max = DB.XYZ(marker_loc.X + width/2 + crop_offset, marker_loc.Z + height + crop_offset, 0)
                    
                    ele1.CropBox = viewBB
                    ele1.CropBoxVisible = False
                    revit.doc.Regenerate()

                    farClip = ele1.get_Parameter(DB.BuiltInParameter.VIEWER_BOUND_OFFSET_FAR)
                    farClip.SetValueString(str(depth + (offset - depth) + crop_offset) + "'")

                    # Tag View
                    tag_mode = DB.TagMode.TM_ADDBY_MULTICATEGORY
                    tag_orient = DB.TagOrientation.Horizontal
                    tag_name = "SSG_Tag_Multicategory"
                    tag_pt = loc.Add(DB.XYZ(0, 0, -.5))

                    eleTag = revit.doc.Create.NewTag(ele1, ele, False, tag_mode, tag_orient, tag_pt)
        ...
        with revit.Transaction("Place views on legend sheet"):
            ...
            first_tile_loc = DB.XYZ(.825/12, 9.5/12, 0)
            first_tile_pt = first_tile_loc.Add(DB.XYZ(tile_width/2, -(tile_length/2), 0))
            ...
            Alert(
                ...
                format(tile_width*12, tile_length*12, total_views, column_qty, row_qty, int(total_sheets), len(source_elements)),\
                title="Legend PDF", header="Legend Tiling"
                )


            view_groups = [views[i:i + int(total_views)] for i in range(0, len(views), int(total_views))]
            order_groups = [order[i:i + int(total_views)] for i in range(0, len(order), int(total_views))]
            descrip_groups = [descrips[i:i + int(total_views)] for i in range(0, len(descrips), int(total_views))]

            sheet_count = 1
            for group, order, descrips in zip(view_groups, order_groups, descrip_groups):
                ...
                for v, o, d in zip(group, order, descrips):
                    ...
                    if o == "First" or o == "Only":
                        note = DB.TextNote.Create(revit.doc, sheet.Id, \
                                                first_tile_pt.Add(DB.XYZ(x, y, 0)).Add(DB.XYZ(-tile_width/2 + .125/12, tile_length/2 + .125/12, 0)), \
                                                "Short Name", text.Id)
                    if o == "Last" or o == "Only":
                        l1 = DB.Line.CreateBound(first_tile_pt.Add(DB.XYZ(x + tile_width/2, y - tile_length/2 + .5/12, 0)), \
                                                first_tile_pt.Add(DB.XYZ(x + tile_width/2, y + tile_length/2 - .25/12, 0)))
                        line_test = revit.doc.Create.NewDetailCurve(sheet, l1)
                        line_test.LineStyle = sep_line


                    vp = DB.Viewport.Create(revit.doc, sheet.Id, v.Id, first_tile_pt.Add(DB.XYZ(x, y, 0)))
