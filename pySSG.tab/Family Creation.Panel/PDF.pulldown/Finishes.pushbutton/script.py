...
components = [Label('Border Size'), 
            ComboBox('combobox1', {'1/8"': .125/12,'1/4"': .25/12, '1/16"': .0625/12, 'None': .001}, default='1/16"'),
            Label('Tag Type'),
            ComboBox('combobox2', list_to_dict_2(mat_tags), default="SSG_Tag_Material_Swatch: Default"),
            Label('Titleblock'),
            ...
            ComboBox('combobox5', list_to_dict(all_text), default='SSG_1/8" - Segoe UI Semibold - Dark Blue'),
            Label('Sheet Header'),
...
if form.show():
    ...
    if res:
        ...
        key_func = lambda x: x.get_Parameter(DB.BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS).AsString()
        ...
        key_func_2 = lambda x: x.get_Parameter(DB.BuiltInParameter.ALL_MODEL_DESCRIPTION).AsString()
        with revit.Transaction("Create Material Swatches"):
            ...
            for group in mat_groups:
                ...
                for mat in group:
                    swatch = revit.doc.Create.NewFamilyInstance(DB.XYZ(x, 0, 0), swatch_fam_type, DB.Structure.StructuralType.NonStructural)
                    x += 2
                    ...
                    aCrop = plan1.get_Parameter(DB.BuiltInParameter.VIEWER_ANNOTATION_CROP_ACTIVE)
                    aCrop.Set(True)
        ...
        with revit.Transaction("Place views on finish sheet"):
            ...
            Alert(
                ...
                format(tile_width*12, tile_length*12, total_views, column_qty, row_qty),\
                title="Finish Options PDF", header="Swatch Tiling"
                )
            ...
            for v, n in zip(views, group_nums):
                if i == 0:
                    ...
                    note = DB.TextNote.Create(revit.doc, sheet.Id, \
                                                first_tile_pt.Add(DB.XYZ(0, (row-1) * -tile_length, 0))\
                                                .Add(DB.XYZ(-tile_width/2, tile_length/2 + .25/12, 0)),\
                                                group_names[group_id], text.Id)
                elif view_count < 1:
                    col = 1
                    row = 1
                    sheet_count += 1
